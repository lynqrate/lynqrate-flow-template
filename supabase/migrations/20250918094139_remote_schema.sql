alter table "public"."user_passes" alter column "purchased_at" set not null;
...
AS $function$
  with me as (
    select user_id, pass_id, purchased_at, created_at, id
    from public.user_passes where id = p_user_pass_id
  )
  select up2.id
  from public.user_passes up2
  join me on me.user_id = up2.user_id
  where (coalesce(up2.purchased_at, up2.created_at), up2.created_at, up2.id)
        < (coalesce(me.purchased_at,  me.created_at),  me.created_at,  me.id)
    and (p_same_kind is false or up2.pass_id = me.pass_id)
  order by coalesce(up2.purchased_at, up2.created_at) desc,
           up2.created_at desc,
           up2.id desc
  limit 1;
$function$
