drop function if exists "public"."save_feedback_and_finish"(p_entry_id uuid, p_sid text, p_feedback_text text, p_gpt_responses jsonb, p_language text);

alter table "public"."user_passes" add column "first_seen_at" timestamp with time zone;
...
declare
  ...
  -- 결과
  v_prev_digest text;
  v_prev_digest_source text; -- 디버깅용 내부 변수(반환엔 포함 안 함)
  v_recent jsonb := '[]'::jsonb;
  v_recent_cnt int := 0;
  v_last_entry_no int := 0;
  v_digest_len int := 0;
...
begin
  ...
  if p_debug then
    ...
    values ('get_rollup_context', 'start', p_user_pass_id, null,
  ...
  if p_debug then
    insert into public.rpc_debug_log(func, phase, user_pass_id, msg, data)
    values ('get_rollup_context', 'meta', p_user_pass_id, null,
            jsonb_build_object(
              'user_id', v_user_id,
  ...
  -- 2) prev/curr 롤업 원천값 수집 (권한/RLS는 SECURITY DEFINER로 우회)
  ...
  select coalesce(prd.digest_text,''), coalesce(prd.last_entry_no,0)
  ...
  if p_debug then
    insert into public.rpc_debug_log(func, phase, user_pass_id, msg, data)
    values ('get_rollup_context', 'sources', p_user_pass_id, null,
            jsonb_build_object(
              'prev_id', v_prev_id,
  ...
  if p_debug then
    insert into public.rpc_debug_log(func, phase, user_pass_id, msg, data)
    values ('get_rollup_context', 'decision', p_user_pass_id, null,
            jsonb_build_object(
              'entry_no_next', v_entry_no_next,
  ...
  if p_debug then
    insert into public.rpc_debug_log(func, phase, user_pass_id, msg, data)
    values ('get_rollup_context', 'end', p_user_pass_id, null,
            jsonb_build_object(
...
with pass_info as (
  select
    ...
    p.total_uses,
    -- 다음 엔트리 번호(1~total_uses 범위로 보정)
    greatest(1, least(p.total_uses, (p.total_uses - coalesce(up.remaining_uses, 0) + 1))) as entry_no_next
  from public.user_passes up
  join public.passes p on p.id = up.pass_id
  where up.id = p_user_pass_id
),
...
curr_prd as (
  select
    ...
    coalesce(last_entry_no,0) as last_entry_no
...
select jsonb_build_object(
  ...
  -- 2) 그 외(entry_no_next>=2):  이번 패스 롤업 → ''
  'prev_digest',
    case
  ...
  -- 참고: 현재 패스 롤업 메타
  'last_entry_no',  coalesce((select last_entry_no from curr_prd), 0),
