alter table "public"."emotion_entries" add column "updated_at" timestamp with time zone not null default now();

alter table "public"."passes" add column "updated_at" timestamp with time zone not null default now();

alter table "public"."standard_emotions" add column "created_at" timestamp with time zone not null default now();

alter table "public"."standard_emotions" add column "updated_at" timestamp with time zone not null default now();
...
DECLARE
  ...
  -- WITH 밖에서 사용할 “결과 요약” 변수들
  v_dec_happened   boolean := false;  -- dec_pass가 행을 냈는지(=이번 호출에서 실제 차감 발생)
  v_ar_stats       jsonb := null;     -- ar_insert가 반환한 stats_json(마지막 회차일 때만 존재)
...
BEGIN
  ...
  -- 메인 CTE 블록: 여기에서 필요한 값들을 모두 계산한 뒤 한 번에 변수로 받는다
  WITH
  lock_entry AS (
    -- 엔트리 잠금 및 최초 여부 확보
    SELECT e.user_pass_id, coalesce(e.is_feedback_generated,false) AS already_generated
  ...
  upsert_feedback AS (
    ...
    VALUES (
      gen_random_uuid(), p_entry_id, v_text, v_lang, v_now, v_now
    )
    ON CONFLICT (emotion_entry_id) DO UPDATE
      SET feedback_text  = excluded.feedback_text,
          language       = excluded.language,
  ...
  ar_insert AS (
    -- 5) 마지막 회차면 최종 합본 요약 대기(pending) 요청 1건 생성
    INSERT INTO public.analysis_requests(
      user_pass_id, scope, status, reason, analysis_text,
      stats_json, created_at, updated_at
    )
  ...
  -- ▶ WITH 블록의 결과를 한 번에 변수로 받는다
  SELECT
    (SELECT id FROM upsert_feedback)                                     AS feedback_id,
    coalesce(
      (SELECT remaining_uses FROM dec_pass LIMIT 1),
      (SELECT up.remaining_uses FROM public.user_passes up
         JOIN lock_entry le ON up.id = le.user_pass_id)
    )                                                                    AS rem_after,
    EXISTS(SELECT 1 FROM dec_pass)                                       AS dec_happened,
    (SELECT stats_json FROM ar_insert LIMIT 1)                            AS ar_stats
  INTO v_feedback_id, v_rem_after, v_dec_happened, v_ar_stats;

  -- 마지막 회차 판정
  v_is_last := v_dec_happened AND v_rem_after = 0;

  -- 마지막 회차일 때만 carryover/current를 반환 세팅
  IF v_is_last AND v_ar_stats IS NOT NULL THEN
    v_prev_carryover := v_ar_stats->>'prev_carryover';
    v_current_digest := v_ar_stats->>'current_digest';
  END IF;

  -- === GPT 'feedbacks' 로그 적재 (실패해도 본 흐름 영향 없음) ===
  BEGIN
    IF p_gpt_responses IS NOT NULL AND (p_gpt_responses ? 'feedbacks') THEN
      -- 배열이면 첫 요소만 사용
      IF jsonb_typeof(p_gpt_responses->'feedbacks') = 'array' THEN
