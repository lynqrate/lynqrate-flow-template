...
CREATE OR REPLACE FUNCTION "public"."issue_email_verify_token"("p_user_id" "uuid", "p_email" "text", "p_ip" "inet" DEFAULT NULL::"inet", "p_ua" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
  ...
  v_email    text;
  v_token    text;
  v_row      public.email_verify_tokens%rowtype;
  v_now      timestamptz := now();
  v_count_1h int;
  v_count_1d int;
begin
  ...
  if v_verified is not null then
    return jsonb_build_object('ok', false, 'reason', 'already_verified', 'email', v_verified);
  end if;

  -- 1) 이메일 정규화/검증
  v_email := public.normalize_and_validate_email(p_email);
  if v_email is null then
    return jsonb_build_object('ok', false, 'reason', 'invalid_email');
  end if;

  -- 2) 후보 이메일 업데이트(최초/변경 의사 반영)
  update public.users
     set email_pending = v_email,
         updated_at    = v_now
   where id = p_user_id;

  -- 3) 쿨다운: 직전 60초 내 발급된 미사용·미만료 토큰이 있으면 재사용
  select * into v_row
    from public.email_verify_tokens
   where user_id = p_user_id
     and used_at is null
     and v_now < expires_at
     and issued_at > (v_now - interval '60 seconds')
  ...
  select count(*) into v_count_1h
    from public.email_verify_tokens
   where user_id = p_user_id
     and issued_at > (v_now - interval '1 hour');

  select count(*) into v_count_1d
    from public.email_verify_tokens
   where user_id = p_user_id
     and issued_at > (v_now - interval '1 day');

  if v_count_1h >= 5 or v_count_1d >= 10 then
    return jsonb_build_object('ok', false, 'reason', 'rate_limited');
  end if;

  -- 5) 아직 유효한 미사용 토큰 있으면 그걸 재사용(쿨다운 창은 지났지만 멱등성 유지)
  select * into v_row
    from public.email_verify_tokens
   where user_id = p_user_id
     and used_at is null
  ...
  insert into public.email_verify_tokens(user_id, email, token, ip, user_agent)
  values (p_user_id, v_email, v_token, p_ip, p_ua)
...
ALTER FUNCTION "public"."issue_email_verify_token"("p_user_id" "uuid", "p_email" "text", "p_ip" "inet", "p_ua" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."verify_email_with_token"("p_token" "text") RETURNS "jsonb"
    ...
    AS $$
declare
  v_evt  public.email_verify_tokens%rowtype;
  v_user public.users%rowtype;
begin
  -- 1) 토큰 로드 + 잠금(경쟁 방지) + 용도/만료/미사용 조건
  select *
    into v_evt
    from public.email_verify_tokens
   where token = p_token
  ...
  select *
    into v_user
    from public.users
   where id = v_evt.user_id
   for update;
  if not found then
    return jsonb_build_object('ok', false, 'reason', 'user_not_found');
  end if;

  -- 3) 이미 인증된 경우(멱등 가드: 토큰은 소비하지 않음)
  if v_user.email_verified is not null then
    return jsonb_build_object('ok', false, 'reason', 'already_verified', 'email', v_user.email_verified);
  end if;

  -- 4) 최신 토큰 보장: pending이 있고, 토큰 email과 다르면 더 최신 토큰이 발급된 것으로 간주
  if v_user.email_pending is not null and v_user.email_pending is distinct from v_evt.email then
    return jsonb_build_object('ok', false, 'reason', 'superseded_token');
  end if;

  -- 5) 확정(pending -> verified) + 토큰 1회용 소진(같은 트랜잭션)
  update public.users
     set email_verified    = v_evt.email,
         email_verified_at = now(),
         email_pending     = null,
         updated_at        = now()
   where id = v_user.id;

  update public.email_verify_tokens
     set used_at = now()
   where id = v_evt.id
     and used_at is null;

  return jsonb_build_object('ok', true, 'user_id', v_user.id, 'email', v_evt.email);
end;
$$;
...
ALTER TABLE "public"."email_verify_tokens" OWNER TO "postgres";
...
COMMENT ON COLUMN "public"."emotion_entries"."user_id" IS '직접 연결된 사용자';



COMMENT ON COLUMN "public"."emotion_entries"."raw_emotion_text" IS '사용자 입력 원본';



COMMENT ON COLUMN "public"."emotion_entries"."supposed_emotion_text" IS 'GPT가 추정한 감정';



COMMENT ON COLUMN "public"."emotion_entries"."standard_emotion_id" IS 'GPT 분류 결과';



COMMENT ON COLUMN "public"."emotion_entries"."standard_emotion_reasoning" IS 'GPT가 왜 그렇게 분류했는지';



COMMENT ON COLUMN "public"."emotion_entries"."situation_raw_text" IS '상황 원문 (항상 저장)';



COMMENT ON COLUMN "public"."emotion_entries"."journal_raw_text" IS '일기 원문 (항상 저장)';
...
COMMENT ON TABLE "public"."emotion_feedbacks" IS 'GPT 피드백 결과';
...
COMMENT ON TABLE "public"."passes" IS '이용권 종류';



COMMENT ON COLUMN "public"."passes"."name" IS '1회권, 10회권';



COMMENT ON COLUMN "public"."passes"."total_uses" IS '총 사용 가능 횟수';



COMMENT ON COLUMN "public"."passes"."expires_after_days" IS '만료일 계산용';



CREATE TABLE IF NOT EXISTS "public"."revisit_keys" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_pass_id" "uuid" NOT NULL,
    "code" "text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE "public"."revisit_keys" OWNER TO "postgres";
...
COMMENT ON TABLE "public"."standard_emotions" IS '표준 감정 정의';



COMMENT ON COLUMN "public"."standard_emotions"."name" IS '기쁨, 슬픔 등';
...
COMMENT ON COLUMN "public"."submission_history"."result_reason" IS 'not_found, expired, no_uses, inactive, exception';



COMMENT ON COLUMN "public"."submission_history"."latency_ms" IS '응답까지 걸린 시간(ms)';
...
COMMENT ON COLUMN "public"."submission_state"."sid" IS 'Tally 폼 고유 ID';



COMMENT ON COLUMN "public"."submission_state"."emotion_entry_id" IS 'emotion_entries.id';



COMMENT ON COLUMN "public"."submission_state"."uuid_code" IS 'user_passes.uuid_code';
...
COMMENT ON TABLE "public"."user_passes" IS '이용권 구매 기록. User:Pass = M:N 구조를 해결하는 중간 테이블';



COMMENT ON COLUMN "public"."user_passes"."remaining_uses" IS '잔여 회차';



COMMENT ON COLUMN "public"."user_passes"."expires_at" IS '구매일 기준 30일 후 만료';



COMMENT ON COLUMN "public"."user_passes"."uuid_code" IS '이용권 인증 코드';
...
ALTER TABLE ONLY "public"."email_verify_tokens"
    ADD CONSTRAINT "email_verify_tokens_token_key" UNIQUE ("token");
...
ALTER TABLE ONLY "public"."revisit_keys"
    ADD CONSTRAINT "revisit_keys_code_key" UNIQUE ("code");



ALTER TABLE ONLY "public"."revisit_keys"
    ADD CONSTRAINT "revisit_keys_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_evt_token" ON "public"."email_verify_tokens" USING "btree" ("token");



CREATE INDEX "idx_evt_user" ON "public"."email_verify_tokens" USING "btree" ("user_id");
...
CREATE INDEX "ix_revisit_keys_code" ON "public"."revisit_keys" USING "btree" ("code");
...
CREATE UNIQUE INDEX "ux_revisit_keys_user_pass" ON "public"."revisit_keys" USING "btree" ("user_pass_id");
...
ALTER TABLE ONLY "public"."revisit_keys"
    ADD CONSTRAINT "revisit_keys_user_pass_id_fkey" FOREIGN KEY ("user_pass_id") REFERENCES "public"."user_passes"("id") ON DELETE CASCADE;
...
ALTER TABLE "public"."revisit_keys" ENABLE ROW LEVEL SECURITY;
...
REVOKE ALL ON FUNCTION "public"."append_submission_log"("p_sid" "text", "p_msg" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."append_submission_log"("p_sid" "text", "p_msg" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."clean_visible_text"("p_text" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."clean_visible_text"("p_text" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."finalize_carryover_digest"("p_user_pass_id" "uuid", "p_emotion_entry_id" "uuid", "p_carryover_digest" "text", "p_gpt_responses" "jsonb") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."finalize_carryover_digest"("p_user_pass_id" "uuid", "p_emotion_entry_id" "uuid", "p_carryover_digest" "text", "p_gpt_responses" "jsonb") TO "service_role";



REVOKE ALL ON FUNCTION "public"."get_prev_user_pass_id"("p_user_pass_id" "uuid", "p_same_kind" boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."get_prev_user_pass_id"("p_user_pass_id" "uuid", "p_same_kind" boolean) TO "service_role";



REVOKE ALL ON FUNCTION "public"."get_rollup_context"("p_user_pass_id" "uuid", "p_limit" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."get_rollup_context"("p_user_pass_id" "uuid", "p_limit" integer) TO "service_role";



REVOKE ALL ON FUNCTION "public"."get_rollup_context"("p_user_pass_id" "uuid", "p_limit" integer, "p_debug" boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."get_rollup_context"("p_user_pass_id" "uuid", "p_limit" integer, "p_debug" boolean) TO "service_role";



REVOKE ALL ON FUNCTION "public"."ingest_entry_and_rollup"("p_sid" "text", "p_user_pass_id" "uuid", "p_user_id" "uuid", "p_entry" "jsonb", "p_gpt_responses" "jsonb", "p_new_digest" "text", "p_ip" "inet", "p_user_agent" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."ingest_entry_and_rollup"("p_sid" "text", "p_user_pass_id" "uuid", "p_user_id" "uuid", "p_entry" "jsonb", "p_gpt_responses" "jsonb", "p_new_digest" "text", "p_ip" "inet", "p_user_agent" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."init_validate_and_attach_user"("p_sid" "text", "p_uuid_code" "text", "p_required" "jsonb", "p_ip" "inet", "p_user_agent" "text", "p_latency_ms" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."init_validate_and_attach_user"("p_sid" "text", "p_uuid_code" "text", "p_required" "jsonb", "p_ip" "inet", "p_user_agent" "text", "p_latency_ms" integer) TO "service_role";



REVOKE ALL ON FUNCTION "public"."issue_email_verify_token"("p_user_id" "uuid", "p_email" "text", "p_ip" "inet", "p_ua" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."issue_email_verify_token"("p_user_id" "uuid", "p_email" "text", "p_ip" "inet", "p_ua" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."log_ai_task_run"("p_entry_id" "uuid", "p_task_type" "text", "p_model" "text", "p_request_id" "text", "p_prompt_tokens" integer, "p_completion_tokens" integer, "p_temperature" numeric, "p_status" "text", "p_error_code" "text", "p_prompt_version" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."log_ai_task_run"("p_entry_id" "uuid", "p_task_type" "text", "p_model" "text", "p_request_id" "text", "p_prompt_tokens" integer, "p_completion_tokens" integer, "p_temperature" numeric, "p_status" "text", "p_error_code" "text", "p_prompt_version" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."mark_submission_fail"("p_sid" "text", "p_reason" "text", "p_history_id" bigint, "p_error_json" "jsonb", "p_ip" "inet", "p_user_agent" "text", "p_latency_ms" integer, "p_emotion_entry_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."mark_submission_fail"("p_sid" "text", "p_reason" "text", "p_history_id" bigint, "p_error_json" "jsonb", "p_ip" "inet", "p_user_agent" "text", "p_latency_ms" integer, "p_emotion_entry_id" "uuid") TO "service_role";



REVOKE ALL ON FUNCTION "public"."normalize_and_validate_email"("p_email" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."normalize_and_validate_email"("p_email" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."normalize_whitespace"("p_text" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."normalize_whitespace"("p_text" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."save_feedback_and_finish_v2"("p_entry_id" "uuid", "p_sid" "text", "p_feedback_text" "text", "p_gpt_responses" "jsonb", "p_language" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."save_feedback_and_finish_v2"("p_entry_id" "uuid", "p_sid" "text", "p_feedback_text" "text", "p_gpt_responses" "jsonb", "p_language" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."seed_and_record_submission"("p_sid" "text", "p_uuid_code" "text", "p_user_pass_id" "uuid", "p_user_id" "uuid", "p_reason" "text", "p_ip" "inet", "p_user_agent" "text", "p_latency_ms" integer) FROM PUBLIC;
...
GRANT ALL ON TABLE "public"."ai_task_runs" TO "service_role";



GRANT ALL ON TABLE "public"."analysis_requests" TO "service_role";



GRANT ALL ON TABLE "public"."email_verify_tokens" TO "service_role";



GRANT ALL ON TABLE "public"."emotion_entries" TO "service_role";



GRANT ALL ON TABLE "public"."emotion_feedbacks" TO "service_role";



GRANT ALL ON TABLE "public"."one_time_email_deliveries" TO "service_role";



GRANT ALL ON TABLE "public"."pass_rollup_digests" TO "service_role";
...
GRANT ALL ON TABLE "public"."revisit_keys" TO "service_role";



GRANT ALL ON TABLE "public"."rpc_debug_log" TO "service_role";



GRANT ALL ON SEQUENCE "public"."rpc_debug_log_id_seq" TO "service_role";



GRANT ALL ON TABLE "public"."standard_emotions" TO "service_role";



GRANT ALL ON TABLE "public"."submission_history" TO "service_role";



GRANT ALL ON SEQUENCE "public"."submission_history_id_seq" TO "service_role";



GRANT ALL ON TABLE "public"."submission_state" TO "service_role";
