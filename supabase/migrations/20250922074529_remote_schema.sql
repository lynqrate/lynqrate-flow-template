alter table "public"."pass_rollup_digests" drop constraint "pass_rollup_digests_user_pass_id_key";

drop index if exists "public"."pass_rollup_digests_user_pass_id_key";

alter table "public"."pass_rollup_digests" add column "entry_no" integer;

alter table "public"."user_passes" add column "rollup_seed_text" text;
...
AS $function$declare
  ...
  v_recent_cnt int := 0;
  v_last_entry_no int := 0;  -- ← 최신 회차 번호 (entry_no)
  v_digest_len int := 0;
...
begin
  ...
  -- 직전 패스 id
  v_prev_id := coalesce(v_prev_id, public.get_prev_user_pass_id(p_user_pass_id, false));

  -- 직전 carryover (done AR 최신 1건)
