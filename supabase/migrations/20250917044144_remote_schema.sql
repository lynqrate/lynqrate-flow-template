revoke delete on table "public"."rpc_debug_log" from "anon";

revoke insert on table "public"."rpc_debug_log" from "anon";

revoke references on table "public"."rpc_debug_log" from "anon";

revoke select on table "public"."rpc_debug_log" from "anon";

revoke trigger on table "public"."rpc_debug_log" from "anon";

revoke truncate on table "public"."rpc_debug_log" from "anon";

revoke update on table "public"."rpc_debug_log" from "anon";

revoke delete on table "public"."rpc_debug_log" from "authenticated";

revoke insert on table "public"."rpc_debug_log" from "authenticated";

revoke references on table "public"."rpc_debug_log" from "authenticated";

revoke select on table "public"."rpc_debug_log" from "authenticated";

revoke trigger on table "public"."rpc_debug_log" from "authenticated";

revoke truncate on table "public"."rpc_debug_log" from "authenticated";

revoke update on table "public"."rpc_debug_log" from "authenticated";

revoke delete on table "public"."rpc_debug_log" from "service_role";

revoke insert on table "public"."rpc_debug_log" from "service_role";

revoke references on table "public"."rpc_debug_log" from "service_role";

revoke select on table "public"."rpc_debug_log" from "service_role";

revoke trigger on table "public"."rpc_debug_log" from "service_role";

revoke truncate on table "public"."rpc_debug_log" from "service_role";

revoke update on table "public"."rpc_debug_log" from "service_role";
...
select jsonb_build_object(
  ...
  -- 요약(digest)
  'prev_digest',         (select digest_text   from prd),
...
begin
  ...
  ar_insert as (
    -- 마지막 회차면 최종 합본 요약 대기(pending) 요청 1건 생성
    ...
    select
      ...
      '',
      jsonb_build_object(
        -- ① 이전 패스 carryover: 1순위 done.stats_json->>'carryover_digest', 폴백 pass_rollup_digests
        'prev_carryover',
          coalesce(prev_ar.carryover_digest, prev_prd.digest_text, ''),
        -- ② 이번 패스 최종 롤링 요약(마지막 회차 시점의 현재본)
        'current_digest',
          coalesce(curr_prd.digest_text, '')
      ),
      v_now, v_now
    from dec_pass d
    -- 현재 패스
    left join public.user_passes up on up.id = d.user_pass_id

    -- ★ 직전 패스 계산: 같은 user_id에서 purchased_at/created_at/id 역순으로 바로 앞(id) 1건
    left join lateral (
      select prev_id
      from (
        select id,
              lag(id) over (partition by user_id order by purchased_at, created_at, id) as prev_id
    ...
    -- 직전 패스의 완료된 분석요청에서 carryover_digest 추출(최신 1건)
    left join lateral (
    ...
    -- 폴백: 직전 패스의 pass 롤업 다이제스트
    left join public.pass_rollup_digests prev_prd
      on prev_prd.user_pass_id = prev.prev_id

    -- 이번 패스의 현재 롤업 다이제스트(누적본)
    left join public.pass_rollup_digests curr_prd
      on curr_prd.user_pass_id = d.user_pass_id
...
BEGIN
  ...
  bump_rollup AS (
    INSERT INTO public.pass_rollup_digests AS d
      (user_pass_id, digest_text, last_entry_no, created_at, updated_at)
    SELECT le.user_pass_id,
           coalesce(e.rollup_digest_snapshot, ''),
           1, v_now, v_now
    FROM lock_entry le
    JOIN public.emotion_entries e ON e.id = p_entry_id
    WHERE le.already_generated = false
    ON CONFLICT (user_pass_id)
    DO UPDATE SET
      digest_text   = CASE WHEN coalesce(excluded.digest_text,'') = '' THEN d.digest_text ELSE excluded.digest_text END,
      last_entry_no = greatest(d.last_entry_no,0) + 1,
      updated_at    = v_now
    RETURNING 1
  ),
  ...
  ar_insert AS (
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY prd.updated_at DESC NULLS LAST, prd.created_at DESC
      LIMIT 1
    ) prev_prd ON true
  ...
  if coalesce(v_seed,'') <> '' then
    ...
    on conflict (user_pass_id) do update
      set digest_text = excluded.digest_text,
          updated_at  = v_now
    where pass_rollup_digests.last_entry_no = 0;
    v_status := 'seeded';
  else
    v_reason := 'empty_seed';
  end if;
